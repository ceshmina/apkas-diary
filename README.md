...
**場所は任意で、書かない日のほうが多い。** `--location` を省けば既存の値がそのまま残り、外すときだけ `--no-location` を明示する。書いた場所は日別ページに出て、書き出しの frontmatter にも入る。持たない日は、ページにも書き出しにも何も出ない——未指定を「東京」とみなすような既定値は持たせていない。
...
記事の編集画面では、**本文の入力とプレビューが同時に見える**。広い画面（60rem 以上）では左右に並び、狭い画面では「編集」「プレビュー」のタブで切り替える。プレビューは手を止めると追随して更新されるので、確認のために押すものは無い。タイトル・場所・公開状態・保存・その日の写真は、どちらを見ていても画面にある。場所は空のままでよく、空にして保存すれば外れる。
...
キーは `pk = PHOTO#<YYYY-MM-DD>` / `sk = <ファイル名>`。日記のエントリは `ENTRY#<年>` にあり、公開エントリの索引（GSI1）に写真は載らない。**同じテーブルにあるが、エントリの読み取りには現れない。** Markdown の書き出しは、写真が何枚増えても変わらない。公開サイトの生成は目録も読むが（下の「拡大表示に出る撮影機材」）、引くのは**写真のある日ごとに1回**で、1日に何枚貼っても往復は増えない。
...
日別ページの写真をクリック（タップ）して拡大すると、写真の下にその写真を撮った機材が出る。目録に記録された機種とレンズを、カンマ区切りで1行にしたものである。

```
ILCE-7RM5, FE 24-70mm F2.8 GM II
iPhone 15 Pro
```

**レンズの名称は、次のいずれかに当たると出ない。**

- **レンズの名称を出さないと定めた系列で撮った。** `src/lib/equipment.ts` の `MODELS_WITHOUT_LENS_NAME` にある。機種名がそこに並ぶ文字列で始まるものが当たる。

  | 系列 | 当たる機種 |
  | --- | --- |
  | `iPhone` / `XQ-` | iPhone・Xperia |
  | `DSC-` / `RICOH GR` / `LEICA D-Lux` / `X100` | Cyber-shot・GR・D-Lux・X100 系（レンズを選べない機材） |

  機種ではなく**系列**で持っているので、同じ系列の次の機種を買っても足さずに済む。**レンズを交換する機材をここに載せてはいけない。**
- **レンズの名称が機種の名称を含む。** 同じ名前を1行に2度出さない。
- **レンズの名称が、字も数字も含まない。** レンズの接点が無い機材を付けたときの `----` のような値。

一覧を持っているのは、**字面では分けられないから**である。`DSC-RX1RM3` はレンズを選べない機材だが、レンズの名称に `35.0 mm f/2.0` とだけ書く。ところが `NIKON D5600` はレンズを交換する機材でありながら、同じ形（`18.0-55.0 mm f/3.5-5.6` など）で3本を言い分ける。字面が同じで望ましい結果が逆になるので、機材の側で決めるほかない。

一覧に無い機材でも、残り2つの規則が効く。**登録するまで壊れるわけではなく、レンズの名称が出るだけである。**

記録の無い写真には何も出ない。行そのものが現れず、「不明」の類の文言も出ない。写真に説明（`![説明](…)` の `[]`）が添えてあれば、説明の下に、説明より薄く置かれる。

出るのは機種とレンズの名称だけである。**撮影日時も露出も位置情報も、配信物には載らない。** 記録そのものにも位置情報は無い（上記）。機材の名称はページの側が持ち、**配信される画像からは相変わらず何も読み取れない**。

字面はビルドのときに1度だけ決まり、出来上がった1行が本文の `img` に `data-equipment` として載る。ブラウザ側に規則は無い。
...
**旧ホストはもう無い。** 2026-09-23 に、旧アカウント（703602958317）の S3・CloudFront・証明書・変換 Lambda と、`photos.old.apkas.net` の DNS レコードをすべて削除した。旧バケットにしか無かった元写真 9 枚（2024-10-27、日記の無い日のもの）は、削除の前に通常の投入経路でこの基盤へ移してある。**写真の実体が、このリポジトリの管理下にない場所に置かれていることは無くなった。**
...
│   ├── cli/          # エントリ登録・写真投入 CLI
│   ├── styles/       # 体裁の基準（配色・寸法・書体の一覧）と本文要素。編集アプリケーションと共有する
...
- **公開手続きが読むのは公開分だけ、書くのは配信物だけ**。実行ロールに与えた DynamoDB の権限は GSI1 への `Query`（サイトの生成）、ベーステーブルへの `Scan`（`export/` への書き出し）、そして写真の目録への `Query`（拡大表示に出す撮影機材）の3つで、**書き込みは1つも無い**。ビルドが日記を壊す経路が権限の側に存在しない。3つめは変換 Lambda と同じく条件（`dynamodb:LeadingKeys`）で `PHOTO#` の下に限ってあり、**エントリのパーティションはベーステーブルからは引けない**——下書きを含むエントリの取得経路が、コードだけでなく IAM でも閉じている。写真のバケットにも届かない。
...
- **体裁の基準は日記が自分で持つ**。配色・寸法は `src/styles/tokens.css`、本文要素は `src/styles/base.css`、使う書体と太さの一覧は `src/styles/fonts.ts` にあり、公開サイトと編集アプリケーションが共にここを読む。ほかのサイト（ポートフォリオの [apkas.net](https://apkas.net) を含む）の体裁は写しておらず、あちらが変わっても追いかけない。画面を足すたびに色や角丸をその場で決めずに済むことを、ここで担保している。
- **書体は、公開サイトは自分で配り、編集アプリケーションは Google Fonts から読む**。どちらも同じ一覧（`fonts.ts`）から作るので、書体や太さを変えるときは一覧だけを直せば両方に効く。公開サイトは Astro の Fonts API でビルド時に Google Fonts から取り込み、自分のドメイン（`/_astro/fonts/`）から配る。閲覧のたびに Google へ問い合わせないためで、訪問者がどのページを読んだかが書体の配信を通じて第三者に伝わらない。和文の書体は1つの太さが約120の断片に分かれ、訪問者が読むのはそのページに現れる字を含む断片だけである（配信物には全部で約 5.4 MB が載る）。編集アプリケーションまで自分で配ると、書体のファイルが Lambda の配布物に入って直接アップロードの上限（zip 50 MB）に近づく。どちらも取得元は同じなので、プレビューと公開後で書体の版がずれるのは、Google で版が上がってから次に公開するまでのあいだに限られる。代わりに、**公開サイトのビルドは Google Fonts に届かないと失敗する**。配信中のサイトはそのまま残るので、再実行すればよい。

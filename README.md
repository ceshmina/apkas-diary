...
- キーは CLI の `--date` と同じ `YYYY/MM/DD/<ファイル名>` になる。**キーを直接指定する手段はブラウザ側にはない。**
- 上限は **50MB**。超えるものは CLI から入れる。ブラウザ側で押す前に弾くが、それは上乗せであって、無効にすると S3 が断る（生の XML が返る）。
- **生成は非同期である。** 投入した直後は URL を開いても画像は返らない。戻った画面が数秒おきに確かめ、開けるようになったら知らせる。同じキーに入れ直した場合、**前の派生画像が残っていることを「終わった」とは扱わない**ので、古い URL を本文に貼ってしまうことがない。
- 完了すると4つの URL と、本文にそのまま貼れる Markdown（`medium`）が出る。
- 90 秒待っても現れないときは待つのをやめる。**いちばん多い原因は HEIC**（次節）で、その場合は待っても現れない。

投入を許す資格は、その日付の下へ**書き込むことだけ**を、15 分のあいだ許すものである。既にある元写真を読むことも、消すこともできない。

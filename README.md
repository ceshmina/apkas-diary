...
**レンズの名称は、それが機種の名称を含まないときだけ出る。** スマートフォンの `LensModel` は `iPhone 15 Pro back triple camera 6.86mm f/1.78` のように機種名で始まり、レンズの名前というより機種の言い直しになっている。同じ名前を1行に2度出さない。固定レンズの機種も同じ形で落ちる。機種やメーカーの一覧はコードに持っていないので、**はじめて使う機材でも登録の操作は要らない**。

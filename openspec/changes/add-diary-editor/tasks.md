...
- [x] 5.13 staging に `terraform apply` し、リソースが作られること、`https://admin.dev.apkas.net` が証明書の警告なく応答することを確認する（この時点の中身は仮のアーカイブでよい）
  - **追加のみ 20 件、既存リソースへの変更・削除は0件**であることを plan で確認してから適用した。
  - `lambroll` はこの環境（Ubuntu、brew なし）に無かったので、GitHub の release から v1.5.1 の linux_amd64 を `~/.local/bin` に入れた。README の必要なツールの表も直した。
  - `backend.hcl` と `terraform.tfvars` はコミットされないため手元に無く、state バケット（`apkas-diary-tfstate-183295408601`）とアカウント ID から組み立てて作った。
  - **予約同時実行が設定できなかった。** このアカウントの同時実行の上限は初期値の 10 で、未予約分を 10 未満にする予約は API が拒否する（`InvalidParameterValueException`）。つまりどの関数も1つも予約できない。既定を -1（予約しない）に変え、理由をモジュールに書いた。この状態ではアカウントの上限そのものが同時実行の上限として働く。
  - 適用後、`https://admin.dev.apkas.net` が証明書の警告なく応答することを確認した（この時点では仮の中身なので 500）。A / AAAA とも解決する。
  - 仮の中身のログは `Cannot find module 'run'`。レイヤーと `AWS_LAMBDA_EXEC_WRAPPER` が lambroll 側にあるため、Node のランタイムが `run.sh` をモジュールとして読もうとする。**設計どおりの「まだデプロイしていない」状態**だが、メッセージとしては親切でない。
...
- [x] 7.4 `scripts/build-editor.sh` を書く。Astro をビルドし、実行時に必要な依存だけを揃えた配布ディレクトリを作る。毎回作り直す
  - **依存を生成物の参照から絞り込む形を試して、やめた。** 16 個まで減らせたが、Astro の生成物は素の import 文だけでは辿れない参照を持っており（`es-module-lexer` が漏れた）、**足りないことに気づくのが Lambda 上での起動失敗**になった。ルートの `dependencies` をそのまま入れる形に戻した（`--ignore-scripts` を付ければ esbuild の postinstall で落ちる問題は避けられる）。数十 MB を惜しんで、配ってから落ちる形を選ぶ理由がない。
  - **配布物だけで起動できることを、リポジトリの外に写して確かめる**手順をビルドに組み込んだ。リポジトリの中で動かすと Node がひとつ上の `node_modules` まで探しにいくため、入れ忘れが手元では見つからない。実際に `es-module-lexer` を抜いた状態で、この確認が起動失敗を捕まえることを確かめた。
  - **sharp を落とした。** `astro` の依存として入るが、読むのは既定の画像サービスだけで、それを noop に差し替えてある。生成物に sharp への参照が無いことをビルド時に検査したうえで消す。これで 27MB 減る。
  - **Markdown プロセッサ（satteri）は native binding を持つ。** `--os` / `--cpu` の指定に従って1つだけ入るため、在ることをビルド時に確かめる。無いとプレビューだけが実行時に落ちる（読み込みが `renderMarkdown` の中まで遅れるので、起動確認では捕まらない）。手元（linux/x64）で配布物のプレビューまで試すには、この環境向けの binding を足す必要がある。
...
- [x] 7.6 配布物のサイズを確認する。直接アップロードの上限（zip 50MB）に収まらない場合は S3 経由の配置に切り替える
  - 展開 160.7 MB（13,652 ファイル）、**zip 37.0 MB**。上限 50MB に対して 13MB の余裕がある。S3 経由への切り替えは要らない。
  - sharp を残したままだと zip 48.5 MB で、上限まで 1.5MB しかなかった。依存が少し増えるだけでデプロイが通らなくなる位置なので、落とす判断はここが決め手になった。
...
- [x] 7.9 環境名を指定せずに `npm run deploy:editor` を実行し、何もデプロイされないまま失敗することを確認する
  ...
  - あわせて、**リポジトリの外へ写した配布物**に対して検査を全部通し直した（ヘッダ 4 件・認証 26 件・画面 24 件・保存とプレビュー 12 件・公開サイトとの一致 3 件、計 69 件）。
  - 検査ハーネス自体にも穴があった。前の実行のサーバが残っていると、新しい配布物を起動できないまま**古いサーバの応答を新しい結果として読んでしまう**。実際にこれで「直したはずのヘッダが付かない」と誤読した。始める前に必ず落とすようにした。
- [x] 7.10 `npm run deploy:editor -- staging` を実行し、直後に `terraform plan` が差分を出さないことを確認する（`ignore_changes` の並べ忘れがないこと）
  - `terraform plan` は差分なし。`ignore_changes` の一覧に漏れはない。
  - **最初のデプロイは起動に失敗した。** `Cannot find package 'es-module-lexer'`。7.4 で生成物の参照から依存を絞り込んでいたが、Astro の生成物は素の import 文だけでは辿れない参照を持っていた。
  - **手元の検査がこれを見逃した理由**が重要で、配布物をリポジトリの中で動かすと Node がひとつ上の `node_modules` まで探しにいき、入れ忘れた依存が拾えてしまう。以後、検査はリポジトリの外へ写した配布物に対して行うようにした。ビルドにも同じ確認を組み込んである。
  - 直したうえで再デプロイし、設定が仮値のままである旨の 500 が返ること、ログに `aws ssm put-parameter` のコマンドがそのまま出ることを確認した。
...
- [ ] 8.1 許可されたアカウントでログインでき、一覧・作成・編集・公開の切り替え・プレビュー・ログアウトがひととおり動くことを確認する
  - **ブラウザでの Google ログインが要るため、画面の一通りの操作は利用者に確認をお願いしている。**
  - **DynamoDB から公開サイトまでの往復は確認済み。** staging のテーブルに `2026-08-09 新規日記のテスト`（公開）が入った状態で `npm run build -- staging` / `npm run deploy -- staging` を実行し、`https://diary.dev.apkas.net/2026/08/09` とトップページの両方に現れることを確認した。
  - あわせて**下書きが漏れていない**ことも確認した。`2025-12-24 内緒の下書き` は日別ページも月別ページも 404 で（公開分が無い月のパスはそもそも生成されない）、年別ページにもトップにも現れない。sparse index による除外が実データで効いている。
  - 1.1 で切り出した CSS（配色・字送り・本文要素）が配信物に載っていることも確認した。
...
- [x] 8.5 実行ロールの権限で `DeleteItem` が拒否されることを確認する（AWS CLI で実行ロールを引き受けて試す）
  - **引き受けでは試せなかった。** ロールの信頼ポリシーは `lambda.amazonaws.com` のみを許しており、こちらの SSO の身元では assume できない。代わりに `aws iam simulate-principal-policy` でロールのポリシーを評価した。こちらのほうが、権限そのものを直接見ている分だけ確か。
  - `DeleteItem` / `BatchWriteItem` / `UpdateItem` はいずれも `implicitDeny`。`GetItem` / `PutItem` / `Scan` は `allowed`。
- [x] 8.6 実行ロールで他方の環境のテーブル、および写真・公開サイトのバケットへの操作が拒否されることを確認する
  - production のテーブルへの `GetItem` / `PutItem`、公開サイトのバケットへの `PutObject`、写真のバケットへの `GetObject` / `PutObject`、CloudFront の `CreateInvalidation` — すべて `implicitDeny`。
- [x] 8.7 十分な時間を置いてからアクセスし、コールドスタートを含む最初の応答が 30 秒以内に返ることを実測する。結果に応じて `MemorySize` を詰める
  - **初期化 836ms、応答まで合計 1.55 秒**（TLS と DNS を含む実測）。上限 30 秒に対して大きく余裕がある。
  - 使用メモリは 1024 MB 中 150 MB。**メモリは 1024 のままにする。** 割り当てを下げると CPU も下がって初期化が伸び、課金は GB 秒なので安くなるとは限らない。この規模では詰める意味が小さい。
  - この実測は設定が未投入の経路（SSM まで到達して 500 を返す）で取ったもの。初期化と SSM の読み取りという重い部分は通っている。
- [x] 8.8 2回目以降の要求で起動の待ちが生じないことを確認する
...
- [x] 10.1 README の冒頭の図に編集アプリケーションの経路を足す
  - CLI とブラウザの2つの入口が同じ DynamoDB に向かうこと、公開サイトへの反映が別の操作であることが図から読めるようにした。
- [x] 10.2 初期セットアップに、Google Cloud での OAuth クライアントの作成と SSM への実値の投入を足す。**これはコード管理の外に置く例外**であることと、その理由を書く
  - 例外にする理由（Terraform の Google プロバイダで扱える OAuth クライアントが IAP のブランドに紐づくものに限られ、個人アカウントで作るウェブアプリケーションのクライアントを作れない）を書いた。
  - **production のリダイレクト URI に localhost を登録しない**ことを明記した。手元から本番の認証を通せる経路を作らない。
- [x] 10.3 日々の運用に、編集アプリケーションの URL と使い方、および「公開サイトへの反映は従来どおり手元の `npm run build` / `npm run deploy`」であることを書く
  - 削除できないこと、コールドスタートで待つこと、使わない月の費用がゼロであることも書いた。手元で動かす手順（認証を迂回しない）も。
- [x] 10.4 環境ごとの表（URL の一覧）に編集アプリケーションの行を足す
  - あわせて Google Cloud プロジェクトの対応表も足した。AWS の profile だけでは環境が決まらなくなったため。
- [x] 10.5 ディレクトリ構成に `editor/` と `terraform/modules/editor/` を足す
  - `editor/` がルートの `package.json` を共有し、Astro のルートもリポジトリのルートのままである理由（`src/lib` と `src/styles` の共有、版のずれの防止）を添えた。
- [x] 10.6 設計上の要点に、「削除できないことを IAM で担保している」「CloudFront ではなく API Gateway を選んだ理由」「使わないあいだに費用の出る構成要素を持たない」を足す
  - 加えて「編集アプリケーションは唯一の入口ではない」「プレビューと公開結果は同じコードを通る」も足した。
- [x] 10.7 秘密の入れ替え（署名鍵の差し替え、クライアント secret の再発行）の手順を書く
  - **関数は起動時に一度だけ SSM を読む**ため、値を入れ替えただけでは動いているインスタンスに反映されない。配り直しが要ることを手順に含めた。
  - 秘密が漏れたときは Google Cloud 側での無効化が先であることも書いた。SSM を書き換えても、古い secret が生きているあいだは他所から使える。
- [x] 10.8 `config/staging.env.example` / `config/production.env.example` に、編集アプリケーションのために増えた項目があれば足す
  - **増える項目は無かった。** 秘密は SSM にあり、URL とパラメータのパスは `function.jsonnet` と `dev-editor.sh` が決める。「ここに足すものはない」ことと、その理由をコメントとして書いた。何も書かないと、次に読む人が転記漏れを疑うことになる。

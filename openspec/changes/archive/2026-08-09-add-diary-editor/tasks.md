...
- [x] 8.1 許可されたアカウントでログインでき、一覧・作成・編集・公開の切り替え・プレビュー・ログアウトがひととおり動くことを確認する
  - 利用者がブラウザで確認。監査の記録にも `{"event":"granted","email":"shu@apkas.net"}` が残っている。書かれた `2026-08-09 新規日記のテスト` が公開サイトまで出たことは下記のとおり。
...
- [x] 8.2 許可されていない Google アカウントでログインを試み、拒否され、エントリの内容がいっさい返らないことを確認する
  - 利用者がブラウザで確認。セッションは発行されず、エントリの内容は返っていない。
  - **拒否したのは Google 側で、こちらの許可リストまでは届いていない。** 記録に残ったのは `{"event":"denied","reason":"google が拒否しました: access_denied"}` で、`許可されていないアカウント` ではない。同意画面がテスト中で、そのアカウントがテストユーザに入っていないためと見られる（同意しなかった場合も同じ `access_denied` になるので、記録からは区別できない）。
  - つまり**関門は2つあり、外側だけが実際に働いた**。内側の許可リストは、Google が通した後にしか出番がない。そちらは `isAllowed` を直接動かして確かめた（大文字小文字と前後の空白は無視する／`shu@apkas.net.evil.com`・`evil.com/shu@apkas.net`・`shu+admin@apkas.net`・空文字はいずれも false）。
  - 内側を実地で働かせるには、同意画面のテストユーザに別アカウントを足して callback まで到達させることになる。**確かめるために外側の関門を緩める操作**なので、ここではやらない。
...
- [x] 9.6 production で 8.1〜8.4 と同じ確認を行う
  - ログインは利用者がブラウザで確認。監査の記録に `granted`（`shu@apkas.net`）が残っている。**許可されていないアカウントの拒否は 8.2 と同じく Google 側で起きており、production の記録に `denied` は無い。**
  - 実際に編集アプリケーションから書かれた 2026-08-09 のエントリを `npm run build -- production` / `npm run deploy -- production` で配信し、[https://diary.apkas.net/2026/08/09](https://diary.apkas.net/2026/08/09) に本文と写真2枚が出ること、トップ・年別・月別のいずれからも辿れることを確認した。**ブラウザで書いたものが公開サイトに出るまでが production で通った。**
  - 以下は実機で確認した。

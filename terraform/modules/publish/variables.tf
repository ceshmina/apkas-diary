...
variable "gsi1_name" {
  description = <<-EOT
    ...
    サイトの生成がエントリを読むのはこの索引だけで、エントリのパーティションは
    読まない。実行ロールの Query をこの索引の ARN に限れるのはそのため。

    ベーステーブルへの Query も1つあるが、対象は写真の目録（PHOTO#<日付>）に
    限ってあり、エントリには届かない（main.tf の QueryPhotoCatalog）。
  EOT
  type        = string
}
